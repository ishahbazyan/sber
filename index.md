...
4. Предоставление Суперадминстратором доступа в АРМ Администратора ОСА для Платформенных и Канальных администраторов.
...
Для того чтобы посмотреть логи запросов и ответов к компоненту ОСА необходимо воспользоваться сервисом Просмотр журналов в компоненте Журналирование. Необходимые логи содержатся в **Интеграционном журнале**.
В поиске необходимо указать временной промежуток, в который осуществлялась попытка входа (будьте внимательны с указанием даты и времени). Для Кода подсистемы выберите **ФП Сервис авторизации (AUTHORIZATION)** и URL сервиса — **/ufs-security/rest/v2/priviliges/get**.
Для детализации поиска можно указать Идентификатор сессии пользователя.

После того как нужная запись в журнале обнаружена, необходимо изучить запись.
Ответ на запрос будет содержать все рассчитанные для пользователя привилегии и группы или не будет содержать, в том случае, если для указанных в запросе атрибутах модель авторизации не вычисляется.
...
Для осуществления зашифрованного обмена данными и проверки подлинности запроса с использованием mTLS в конфигурационном файле vs-egress-ott.yaml используется три параметра:

* два из URL: host и port;
...
**Примечание:** выпуск сертификата осуществляет администратор стенда. Все сертификаты будут находиться в хранилище сертификатов ingress и egress на стенде в репозитории common.
...
| ufs-security.ose.egress.ott.grpc.port | ${global.ott.grpc.port}             | Путь unix сокета для egress                                                                                                 |
| ufs-security.ose.egress.ott.service.url | ${global.ott.service.url}           | Шаблон URL сервиса OTT, при балансировке значения будут подставляться из OTT_SERVICE_HOSTS для egress                       |
| ufs-security.ose.egress.ott.service.hosts | ${global.ott.service.hosts}         | Список серверов:портов сервиса OTT для балансировки нагрузки, или аналогичный список nginx OTT (на НТ/ПСИ/Пром) для egress  |
| ufs-security.ose.ingress.ott.grpc.port | ${global.ott.grpc.port}             | Путь unix сокета для ingress                                                                                                |
| ufs-security.ose.ingress.ott.service.url | ${global.ott.service.url}           | Шаблон URL сервиса OTT, при балансировке значения будут подставляться из OTT_SERVICE_HOSTS для ingress                      |
| ufs-security.ose.ingress.ott.service.hosts | ${global.ott.service.hosts}         | Список серверов:портов сервиса OTT для балансировки нагрузки, или аналогичный список nginx OTT (на НТ/ПСИ/Пром) для ingress |
...
Конфигурация клиентского сертификата осуществляется администратором стенда, а серверный сертификат конфигурируется администратором СУБД.
Администратор стенда заносит приватный ключ и клиентский сертификат в репозиторий (common) настроек стенда и создает глобальные переменные с указанием путей к этим файлам.
Далее администратор создает глобальную переменную в конфигурационном файле ufs-security-manager/dc.yaml для строки подключения к БД (пример):
...
### Предоставление Суперадминстратором доступа в АРМ Администратора ОСА для Платформенных и Канальных администраторов

> Необходимые привилегии:
>
> * роль Суперадминистратор.

1. Для платформенной и канальной изоляции будут использоваться роли сервиса аутентификации (например, KeyCloak.SE/СУДИР), поэтому сначала необходимо определить какие новые или имеющиеся роли в сервисе аутентификации вам потребуются. Для изоляции конфигурации в разрезе сектора необходима отдельная роль. Если с помощью вашего приложения изменяются настройки в трех разных канальных секторах, то вам потребуются 4 роли провайдера аутентификации (KeyCloak.SE/СУДИР) — одна роль Суперадминистратора с доступом ко всем конфигурациям, и по одной роли для каждого канального сектора.
2. Для канальной изоляции необходимо внести изменения в файл конфигурации модели авторизации вашего приложения с использованием специализированного фильтра авторизации:
...
Для платформенной изоляции указать параметр тенант-кода при загрузке ролевой модели.

3. После создания необходимых ролей для изоляции, Суперадминистратор заходит в консоль оператора компонента ОСА и загружает ролевые модели через кнопку **Импорт**.
4. Для ограничения доступа (вкладка/действие) в консоли оператора компонента ОСА Суперадминистратор переходит на вкладку **Набор привилегий**.
5. Через расширенный поиск ищет в поле Наименование ищет недавно загруженный набор привилегий для канального или платформенного администратора.
6. Чтобы отредактировать найденный набор привилегий, необходимо напротив в поле Действие нажать кнопку **Редактировать набор привилегий** (пиктограмма Зеленый карандаш).
7. В разделе Просмотр прав набора привилегий в строке поиска наберите «Управление правами доступа», чтобы появился список привилегий АРМа администратора.
...
9. В выпадающем списке «снимите» выбор с «Поддержка и сопровождение Сегмент Сотрудники» и нажмите кнопку **Сохранить** (пиктограмма «галочка»).
10. Набор привилегий для данного пользователя изменен.
...
Компонента ОСА поддерживает 1 тип ресурса: Role (справочник ролей).

Справочник ролей это плоский справочник содержащий идентификаторы и описания ролей. Для получения этого справочника на стороне ОСА предусмотрен endpoint.
Адаптер SCIM использует данный endpoint для получения справочника ролей. В учетной записи АС есть многозначный атрибут для хранения ролей.
...
KeyCloak.SE/СУДИР периодически запрашивает список ролей на сервере и хранит их у себя для того чтобы можно было назначать роли в учетную запись.
...
События системного журнала отправляются и находятся в компоненте Журналирование.
...
Настройка интеграции с компонентом Объединенный мониторинг Unimon описана в документе «[Руководстве по установке](../../installation-guide/md/index.md#Настройка-интеграции-с-компонентом-MONA-Объединенный-мониторинг-Unimon)».
...
* формирования отчетов в любых разрезах и агрегатах;
...
* никаких дополнительных агерегатов до хранилища не предусмотрено;
* подсчет агрегатов делается на этапе предварительной сборки или отчетов.

Такой подход позволяет получить все необходимые агрегаты для сборки и менять их не изменяя архитектуру или компоненты платформы.

В компоненте ОСА используется метрика мониторинга: EVENT_API_CALL. Данная бизнес метрика выполняет подсчет количества запросов к компоненту ОСА для сборки по потребителям.
...
* service: 4-буквенный - код из реестра компонентов: AUTZ;
* region: deploymentGroupId по умолчанию из ENV (или параметров) провайдера. Для глобальных сервисов может отсутствовать;
* account: Идентификатор on-premise инсталляции. По умолчанию из ENV (или параметров) провайдера;
* project: module_id вызывающей ФП из http-заголовка. (см. resource-id, заполняется так же);
* resource-id: module_id вызывающей ФП из http-заголовка. Для получения заголовка, настраиваем ОТТ для проброски subject из токена в заголовок.

## Часто встречающиеся проблемы и пути их устранения

### Не вижу нерассчитанные для пользователя привилегии?

По умолчанию уровень логирования выставлен на ERROR. Нерассчитанные для пользователя привилегии не является ошибкой для компонента ОСА, поэтому для фиксации лога и его последующего анализа, предварительно необходимо выставить в компоненте PACMAN уровень логирования на DEBUG.

### Где можно настроить уровень логирования для компонента ОСА?

Уровень логирования настраивается в компоненте Журналирование, необходимо выставить
logger.remote.level = INFO или logger.name.level = ru.sbrf.ufs.platform.logger.helper.LoggerHttpFilter:INFO.